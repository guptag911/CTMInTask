      ...
      if (comments["commentId"] === cmtid) {
        ...
        try {
          if (comments["status"] === "open" && data["status"] === false) {
            ...
            try {
              var go = await window.gapi.client.tasks.tasklists.insert({
                tasklist: "@default",
                body: new_task,
              });
              data["taskid"] = go["id"];
            ...
            if (data["taskid"] !== null) {
              var task = await window.gapi.client.tasks.tasklists.get({
                tasklist: "@default",
                task: data["taskid"],
              });
              task["status"] = "completed";
              task["hidden"] = true;
              var result = await window.gapi.client.tasks.tasklists.update({
                tasklist: "@default",
                task: task["id"],
                body: task,
              });

...
import { GsuiteDataSave, GsuiteDataGet } from "./gsuiteApi";
...
export const MessageList = async () => {
    ...
    let GFireData = [];
    let GFireDataDict = {}
    GFireData = await GsuiteDataGet();
    console.log("Gfire data is ", GFireData);
    GFireData.forEach(element => {
        GFireDataDict[element.mid]={"taskid":element.taskid, "status":element.status};
    });
        ...
        messages.forEach(async (element) => {
            ...
            try {
                if (userSchema["file_id"]) {
                    ...
                    // console.log("gfg dict is", GFireDataDict, " user schema is ", userSchema);
                    // console.log("GFmid data", GFireDataDict[userSchema["mid"]], " mid ", userSchema["mid"] ,"  ",GFireDataDict[userSchema["mid"]]["taskid"], " user schema is status", GFireDataDict[userSchema["mid"]]["status"]);
                    if(!GFireDataDict[userSchema["mid"]]){

  ...
  try {
    ...
    messages.forEach(async (element) => {
      ...
      try {
        ...
        if (my_url !== null) user_schema["url"] = my_url[0];
      ...
      if (url === null) user_schema["comment_id"] = null;
      else {
        try {
      ...
      try {
        ...
        some_data.forEach(async (comments) => {
          if (comments["commentId"] === comment_ID) {
            ...
            if (title) user_schema["task_desc"] = title;
            else user_schema["task_desc"] = null;
          }
        });
...
console.log("Fixed Gsuite is working");

...
export default function ChartFunc(props) {
  ...
  useEffect(() => {
    (async function anyNameFunction() {
      try {
        let pendTasks = await ConfAnalyticsPendingWithinPeriod(
          fromDate,
          toDate,
          props.open,
          props.id
        );
        let compTasks = await ConfAnalyticsCompletedWithinPeriod(
          fromDate,
          toDate,
          props.open,
          props.id
        );
        let totTasks = await ConfAnalyticsTotalWithinPeriod(
          fromDate,
          toDate,
          props.open,
          props.id

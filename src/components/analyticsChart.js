...
export default function Chart() {
    ...
    // const [gsuiteData, SetGsuiteData] = useState(null);
        ...
        (async function anyNameFunction() {
            try {
                ...
                for (let ele in Hubdata) {
                    if (Hubdata[ele].engagement.type === "TASK" && Hubdata[ele].metadata.status === "COMPLETED") {
                        compTasks += 1;
                        totTasks += 1;
                    }
                    else if (Hubdata[ele].engagement.type === "TASK" && Hubdata[ele].metadata.status !== "COMPLETED") {
                        pendTasks += 1;
                        totTasks += 1;
                    }
                }
